...
: rclcpp_lifecycle::LifecycleNode("ahrs_bringup"),
  ...
  updater_(this), consecutive_read_failures_(0)
{
  // topic_name & frame_id
...
auto ahrsBringup::on_activate(const rclcpp_lifecycle::State & /*previous_state*/) -> ahrsBringup::CallbackReturn
{
  ...
  update_timer_ = this->create_wall_timer(
    std::chrono::duration<double>(1.0 / frequency_), std::bind(&ahrsBringup::processLoop, this));
  return ahrsBringup::CallbackReturn::SUCCESS;
}
...
void ahrsBringup::processLoop()
{
  ...
  if (
    consecutive_read_failures_ > get_parameter("max_consecutive_read_failures").as_int() &&
    get_current_state().id() == lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
    trigger_transition(lifecycle_msgs::msg::Transition::TRANSITION_DEACTIVATE);
  }
  // check head start
  ...
  if (head_type[0] == TYPE_AHRS && check_len[0] != AHRS_LEN)
  {
    RCLCPP_WARN(this->get_logger(), "head_len error (ahrs)");
    consecutive_read_failures_++;
    return;
  }
  if (head_type[0] == TYPE_INSGPS && check_len[0] != INSGPS_LEN)
  {
    RCLCPP_WARN(this->get_logger(), "head_len error (insgps)");
    consecutive_read_failures_++;
    return;
  }
  if (head_type[0] == TYPE_GROUND || head_type[0] == 0x50)  // 未知数据，防止记录失败
  {
  ...
  if (head_type[0] == TYPE_IMU)
  {
    ...
    if (imu_frame_.frame.frame_end != FRAME_END)
    {
  ...
  else if (head_type[0] == TYPE_AHRS)
  {
    ...
    if (ahrs_frame_.frame.frame_end != FRAME_END)
    {
      RCLCPP_WARN(this->get_logger(), "check frame end.");
      consecutive_read_failures_++;
      return;
  ...
  else if (head_type[0] == TYPE_INSGPS)
  {
    ...
    if (insgps_frame_.frame.frame_end != FRAME_END)
    {
      RCLCPP_WARN(this->get_logger(), "check frame end.");
      consecutive_read_failures_++;
      return;
